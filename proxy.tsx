import { auth, clerkClient, clerkMiddleware, currentUser } from '@clerk/nextjs/server';
...
export default clerkMiddleware(async(auth) => {
  ...
  if(userId && !orgId){
    try {
      const client = await clerkClient();   // clerk backend api client, gives permission to interact with backend
      
      //check is user has any organization
      ...
      console.log("Auto created organisation: ",orgName);
    } catch (error) {
